...
if __name__ == "__main__":
    ...
    for i, s in enumerate(np.linspace(0, 1, n_steps)):
        ...
        if i > 0:
            geodesic_error = geodesic_angle(tmp_path[-2], T_s)
    ...
    # linear_ortho_error = np.load("linear_det.npy")
    # euler_ortho_error = np.load("euler_det.npy")
    # quat_ortho_error = np.load("quat_det.npy")
